...
// ISO (yyyy-mm-dd) to avoid locale headaches at the URL level
...
export function parseTxQuery(raw: Record<string, string | string[] | undefined>): TxQueryInput {
// Normalize first: take only first value for arrays
const flat: Record<string, string> = {};
for (const [k, v] of Object.entries(raw)) {
if (Array.isArray(v)) flat[k] = v[0] ?? ""; else if (v != null) flat[k] = v;
}
const parsed = txQuerySchema.safeParse(flat);
if (!parsed.success) {
// Fallback to defaults if malformed input is present
return txQuerySchema.parse({});
}
return parsed.data;
}
